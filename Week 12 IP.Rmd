...
```{r}
# Filtering numeric columns and subsetting the dataframe


#data_num <- subset(data, select = -c( Ad.Topic.Line, City,	Male,	Country,	#Timestamp,	Clicked.on.Ad))
#head(data_num)
```
...
```
```{r}
#data_num <- subset(df, select = -c(Male, Clicked.on.Ad))
#head(data_num)
```

```{r}
# Using the boxplot to detect outliers in the numeric columns
boxplot((df$Daily.Time.Spent.on.Site),ylim=c(0,300))

```
```{r}
boxplot((df$Age),ylim=c(0,300))

```


```{r}
boxplot((df$Area.Income),ylim=c(0,300))
